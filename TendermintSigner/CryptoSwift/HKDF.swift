...
public struct HKDF {
    ...
    fileprivate let variant: HMAC_CC.Variant
    ...
    public init(password: Array<UInt8>, salt: Array<UInt8>? = nil, info: Array<UInt8>? = nil, keyLength: Int? = nil /* dkLen */, variant: HMAC_CC.Variant = .sha256) throws {
        ...
        prk = try HMAC_CC(key: salt ?? [], variant: variant).authenticate(password)
        self.info = info ?? []
    ...
    public func calculate() throws -> Array<UInt8> {
        let hmac = HMAC_CC(key: prk, variant: variant)
        var ret = Array<UInt8>()
